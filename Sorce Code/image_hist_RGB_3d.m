function [freq, freq_emph, freq_app,gcf] = image_hist_RGB_3d(imname,n,currentfilename)
gamma = -1;
...
%% Reading the image
disp('Starting to draw 3D histogram')
global I;
im = I; %imread(imname);    % loading the image
s = size(im);
im = double(im);
...
% view([-45, -45,30]);
view([45, 45,-30]);
saveas(gca,['Results\',currentfilename ,'\3dHist.bmp'])
%% Compute FREQ_APP
if nargout>2
